    ...
    >
      ...
      <div className="flex flex-col justify-between lg:col-span-2">
        <div className="relative h-full space-y-4 px-4 py-8 md:p-8">
          <h1 className="text-3xl font-semibold md:text-4xl lg:text-5xl text-white [text-shadow:_0_4px_20px_rgb(0_0_0_/_60%)]">
            {title}
          </h1>
          <p className="max-w-xl text-sm md:text-base lg:text-lg text-gray-300 [text-shadow:_0_2px_10px_rgb(0_0_0_/_50%)] leading-relaxed">
            {description}
          </p>
...
function ContactInfo({ icon: Icon, label, value, className, ...props }: ContactInfoProps) {
  return (
    <div className={cn("flex items-center gap-3 py-3", className)} {...props}>
      ...
      <div>
        <p className="font-medium text-white [text-shadow:_0_2px_8px_rgb(0_0_0_/_40%)]">
          {label}
        </p>
        <p className="text-xs text-gray-300 [text-shadow:_0_2px_6px_rgb(0_0_0_/_40%)]">{value}</p>
      </div>
    </div>
  )
}
